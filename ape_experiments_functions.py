...
def k_closest_experiments(ape_explainer, k_closest):
    ...
    train_data_counterfactual_class = ape_explainer.train_data[index_train_data_counterfactual_class]

    neigh = NearestNeighbors(n_neighbors=k_closest+1, algorithm='ball_tree', metric=distances, metric_params={"ape": ape_explainer})
    ...
    for ennemis in closest_ennemis:
        dists.append(distances(ape_explainer.closest_counterfactual, ennemis, ape_explainer))
        
    mean_dists, _ = neigh.kneighbors(train_data_counterfactual_class, k_closest+1)
...
def compute_all_explanation_method_accuracy(ape_tabular, instance, growing_field, nb_instance_test_data_label_as_target,
    ...
    local_surrogate_extend_raw_accuracy, local_surrogate_extend_raw_coverage, f2_local_surrogate_extend_raw, _, \
                        ape_tabular.extended_radius = ape_tabular.compute_lime_extending_accuracy_coverage(instances_in_field, 
                                            ...
                                            farthest_distance_cf, growing_method, position_instances_in_field)
    ...
    accuracy_local_surrogate['all'] = ape_tabular.compute_linear_regression_accuracy(local_surrogate_prediction, local_surrogate_labels)
    ...
    ape_accuracy = anchor_accuracy if ape_tabular.multimodal_results else local_surrogate_extend_raw_accuracy
    ...
    accuracys = [accuracy_local_surrogate['all'], local_surrogate_extend_raw_accuracy['all'], \
                anchor_accuracy['all'], decision_tree_accuracy['all'], ape_accuracy['all']]
    coverages = [linear_coverage, local_surrogate_extend_raw_coverage, anchor_coverage, decision_tree_coverage, ape_coverage]
    f2s = [f2_linear_surrogate, f2_local_surrogate_extend_raw,  f2_anchor, f2_ape, f2_decision_tree]
    multimodal = 1 if ape_tabular.multimodal_results else 0
    accuracys_real = [accuracy_local_surrogate['real'], local_surrogate_extend_raw_accuracy['real'], \
                anchor_accuracy['real'], decision_tree_accuracy['real'], ape_accuracy['real']]
...
def compute_local_surrogate_accuracy_coverage(ape_tabular, instance, growing_field,
    ...
    accuracy_local_surrogate = ape_tabular.compute_linear_regression_accuracy(prediction_inside_field, labels_testing_instance_in_field)
    ...
    accuracy_local_surogate_raw_data = ape_tabular.compute_linear_regression_accuracy(prediction_inside_field, labels_testing_instance_in_field)
    ...
    accuracy_ls_raw_data, lime_extending_coverage, f2_lime_extending, ls_explanation, radius = ape_tabular.compute_lime_extending_accuracy_coverage(
